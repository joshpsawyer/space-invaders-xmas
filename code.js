...
var SPRITE_SHEET_ALT = "sprites.png";
var BULLET_AUDIO_SRC = '';
var EXPLOSION_EFFECT_SRC = '';
var LEFT_KEY = 37;
var RIGHT_KEY = 39;
var SHOOT_KEY = 88;
var TEXT_BLINK_FREQ = 500;
var PLAYER_CLIP_RECT = { x: 242, y: 631, w: 109, h: 123 };
var ALIEN_BOTTOM_ROW = [{ x: 273, y: 69, w: 51, h: 53 }, { x: 332, y: 69, w: 51, h: 53 }];
var ALIEN_MIDDLE_ROW = [{ x: 266, y: 145, w: 62, h: 57 }, { x: 330, y: 145, w: 62, h: 57 }];
var ALIEN_MIDDLE_2_ROW = [{ x: 271, y: 222, w: 50, h: 52 }, { x: 324, y: 222, w: 50, h: 52 }];
var ALIEN_MIDDLE_3_ROW = [{ x: 270, y: 303, w: 55, h: 53 }, { x: 330, y: 303, w: 55, h: 53 }];
var ALIEN_TOP_ROW = [{ x: 278, y: 384, w: 39, h: 64 }, { x: 320, y: 384, w: 39, h: 64 }];
...
var bulletAudio = null;
var explosionAudio = null;
var playerDeathAudio = null;
var backgroundMusic = null;
var backgroundMusicStarted = false;
var recordScratchAudio = null;
...
function initCanvas() {
  ...
  spriteSheetImg.src = SPRITE_SHEET_ALT;
...
function setupAlienFormation() {
  ...
  for (var i = 0, len = 5 * 11; i < len; i++) {
    ...
    var clipRects;
    switch (gridY) {
      case 4:clipRects = ALIEN_BOTTOM_ROW;break;
      case 3:clipRects = ALIEN_MIDDLE_ROW;break;
      case 2:clipRects = ALIEN_MIDDLE_2_ROW;break;
      case 1:clipRects = ALIEN_MIDDLE_3_ROW;break;
      case 0:clipRects = ALIEN_TOP_ROW;break;
    }
  ...
  for (var i = 0, len = bullets.length; i < len; i++) {
    ...
    for (var j = 0, alen = aliens.length; j < alen; j++) {
      ...
      if (checkRectCollision(bullet.bounds, alien.bounds)) {
        ...
        explosionAudio.get();
        var color = (Math.random()) > 0.5 ? 'gold' : 'silver';
        particleManager.createExplosion(alien.position.x, alien.position.y, color, 70, 5, 5, 3, .15, 50);
        player.score += 25;
      }
    }
  }
  ...
  for (var i = 0, len = aliens.length; i < len; i++) {
    ...
    if (alien.bullet !== null && checkRectCollision(alien.bullet.bounds, player.bounds)) {
      if (player.lives === 0) {
        ...
        recordScratchAudio.play();
        backgroundMusic.stop();
      } else {
        alien.bullet.alive = false;
        playerDeathAudio.get();
...
function drawBottomHud() {
  ...
  26, 45, CANVAS_HEIGHT - 23, player.clipRect.w * 0.5,
  26 * 0.5);
...
function drawStartScreen() {
  fillCenteredText("Holiday Invaders", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2.75, '#FFFFFF', 36);
  fillBlinkingText("Press enter to play!\r\nx to fire!", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2, 500, '#FFFFFF', 36);
}
